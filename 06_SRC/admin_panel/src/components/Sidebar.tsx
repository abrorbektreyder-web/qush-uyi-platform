...
export const Sidebar: React.FC<SidebarProps> = ({ onLogout, onAnalytics }) => {
    return (
        <aside className="sidebar">
            ...
            <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    ...
                    <div>
                        <h2 style={{ color: 'var(--text-main)', fontSize: '1.2rem', fontWeight: 800, letterSpacing: '-0.5px', margin: 0 }}>
                            <span style={{ color: 'var(--primary)' }}>QUSH</span> UYI
                        </h2>
                        <p style={{ color: 'var(--text-muted)', fontSize: '0.68rem', marginTop: '2px', fontWeight: 500, letterSpacing: '1px', textTransform: 'uppercase' }}>
                            Admin Portal 3.0
                        </p>
                    </div>
                </div>
            ...
            <nav style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '2px', padding: '0 12px' }}>
                ...
                <button
                    onClick={onAnalytics}
                    className="sidebar-link analytics-btn"
                    style={{
                        display: 'flex', alignItems: 'center', gap: '14px',
                        padding: '13px 18px', border: 'none', background: 'none',
                        color: 'var(--text-muted)', fontSize: '0.92rem', fontWeight: 500,
                        borderRadius: '12px', cursor: 'pointer', transition: 'all 0.2s',
                        textAlign: 'left', width: '100%', marginTop: '4px',
                    }}
                >
                    <BarChart3 size={20} />
                    <span style={{ flex: 1 }}>Analitika</span>
                    <span style={{
            ...
            <div style={{ padding: '16px 20px', borderTop: '1px solid var(--border-glass)' }}>
                <div style={{
                    ...
                    <div style={{
                        ...
                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                        color: '#0B0D17', fontWeight: 800, fontSize: '0.85rem',
                    }}>
                        AD
                    </div>
                    <div>
                ...
                <button
                    ...
                    style={{ width: '100%', justifyContent: 'center', borderRadius: '12px', padding: '12px', fontSize: '0.85rem' }}
                    onClick={onLogout}
                >
...
const SidebarLink: React.FC<{ to: string; icon: React.ReactNode; label: string; badge?: string }> = ({ to, icon, label, badge }) => (
    <NavLink
        to={to}
        end={to === '/'}
        ...
        style={{
            ...
            color: 'var(--text-muted)', fontSize: '0.92rem', fontWeight: 500,
            borderRadius: '12px', transition: 'all 0.2s', position: 'relative',
        }}
    >
        {icon}
